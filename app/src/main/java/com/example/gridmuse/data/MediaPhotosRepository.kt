...
class LightRoomPhotosRepository(private val context: Context) : MediaPhotosRepository{
  ...
  override suspend fun insertPhotoAtSort(selected: Int, target: Int): Unit = suspendCoroutine { continuous ->
    CoroutineScope(Dispatchers.IO).launch {
      try {
        val photos = insertPhotoAtSortImpl(selected, target)
        continuous.resume(photos)
      }catch (e: Exception) {
        continuous.resumeWithException(e)
      }
  ...
  override suspend fun swapPhotos(selected: Int, target: Int): Unit = suspendCoroutine { continuous ->
    CoroutineScope(Dispatchers.IO).launch {
      try {
        val photos = swapPhotosImpl(selected, target)
        continuous.resume(photos)
      }catch (e: Exception) {
        continuous.resumeWithException(e)
      }
  ...
  override suspend fun getImageList(): List<DevicePhoto> = suspendCoroutine { continuous ->
    CoroutineScope(Dispatchers.IO).launch {
      try {
        val photos = getImageListImpl()
        continuous.resume(photos)
      }catch (e: Exception) {
        continuous.resumeWithException(e)
      }
  ...
  private fun makeJsonSortContinuous():List<DevicePhoto> {
    ...
    //existingPhotos[88].isHidden = true
